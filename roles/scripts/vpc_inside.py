...
# parser.add_argument("-p", '--profile', default='cultureamp', help="AWS profile")
...
def describe_vpc_peerings():
    pcx_accepter = vpc_client.describe_vpc_peering_connections(Filters=[{'Name': 'accepter-vpc-info.vpc-id',
                                                                    'Values': [vpc_id]}])['VpcPeeringConnections']
    pcx_requester = vpc_client.describe_vpc_peering_connections(Filters=[{'Name': 'requester-vpc-info.vpc-id',
                                                                    'Values': [vpc_id]}])['VpcPeeringConnections']
    logger.info("VPC Peering in VPC {}:".format(vpc_id))
    for pcx in pcx_accepter:
        logger.info(pcx["VpcPeeringConnectionId"])
    for pcx in pcx_requester:
        logger.info(pcx["VpcPeeringConnectionId"])
    return
